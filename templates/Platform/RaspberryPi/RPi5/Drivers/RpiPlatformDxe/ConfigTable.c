...
STATIC ACPI_PCIE_32_BIT_BAR_SPACE_SIZE_MB_VARSTORE_DATA
...
AcpiUpdateSdtNameInteger(
    IN EFI_ACPI_DESCRIPTION_HEADER *AcpiTable, IN CHAR8 Name[AML_NAME_SEG_SIZE],
    IN UINTN Value)
{
  UINTN  Index;
...
STATIC
VOID EFIAPI DsdtFixupStatus(
    IN EFI_ACPI_SDT_PROTOCOL *AcpiSdtProtocol, IN EFI_ACPI_HANDLE TableHandle)
{
  EFI_STATUS Status;
  UINTN      Index;
  ...
  for (Index = 0; Index < ARRAY_SIZE(DevStatus); Index++) {
    if (DevStatus[Index].Enabled == FALSE) {
      Status = AcpiAmlObjectUpdateInteger(
          AcpiSdtProtocol, TableHandle, DevStatus[Index].ObjectPath, 0x0);
      if (EFI_ERROR(Status)) {
        DEBUG(
            (DEBUG_ERROR, "%a: Failed to patch %a. Status=%r\n", __func__,
...
STATIC
VOID EFIAPI DsdtFixupSd(
    IN EFI_ACPI_SDT_PROTOCOL *AcpiSdtProtocol, IN EFI_ACPI_HANDLE TableHandle)
{
  EFI_STATUS Status;

  Status = AcpiAmlObjectUpdateInteger(
      AcpiSdtProtocol, TableHandle, "\\_SB.SDCM", AcpiSdCompatMode.Value);
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "%a: Failed to patch AcpiSdCompatMode.\n", __func__));
  }

  Status = AcpiAmlObjectUpdateInteger(
      AcpiSdtProtocol, TableHandle, "\\_SB.SDLU", AcpiSdLimitUhs.Value);
  if (EFI_ERROR(Status)) {
    ...
    IN EFI_ACPI_SDT_PROTOCOL *AcpiSdtProtocol, IN EFI_ACPI_HANDLE TableHandle)
{
  ...
  Status = gBS->LocateHandleBuffer(
      ByProtocol, &gRp1BusProtocolGuid, NULL, &HandleCount, &Handles);
  if (EFI_ERROR(Status)) {
    DEBUG(
        (DEBUG_WARN, "%a: Failed to locate RP1 instance! Status=%r\n", __func__,
         Status));
    return;
  }
  ...
  Status =
      gBS->HandleProtocol(Handles[0], &gRp1BusProtocolGuid, (VOID **)&Rp1Bus);
  FreePool(Handles);

  if (EFI_ERROR(Status)) {
    DEBUG(
        (DEBUG_WARN, "%a: Failed to get RP1 bus protocol! Status=%r\n",
         __func__, Status));
    return;
  }

  Status = AcpiAmlObjectUpdateInteger(
      AcpiSdtProtocol, TableHandle, "\\_SB.RP1B.PBAR",
      Rp1Bus->GetPeripheralBase(Rp1Bus));
  if (EFI_ERROR(Status)) {
    DEBUG(
        (DEBUG_ERROR, "%a: Failed to patch PBAR. Status=%r\n", __func__,
         Status));
  }
}

STATIC
VOID EFIAPI DsdtFixupPcie(
    IN EFI_ACPI_SDT_PROTOCOL *AcpiSdtProtocol, IN EFI_ACPI_HANDLE TableHandle)
{
  EFI_STATUS Status;

  Status = AcpiAmlObjectUpdateInteger(
      AcpiSdtProtocol, TableHandle, "\\_SB.BB32", mAcpiPciMem32Base);
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "%a: Failed to patch BB32.\n", __func__));
  }

  Status = AcpiAmlObjectUpdateInteger(
      AcpiSdtProtocol, TableHandle, "\\_SB.MS32", mAcpiPciMem32Size);
  if (EFI_ERROR(Status)) {
...
EFI_STATUS
EFIAPI
AcpiFixupPcieEcam(IN ACPI_OS_BOOT_TYPE OsType)
{
  EFI_STATUS                   Status;
  UINTN                        Index;
  ...
  Status = AcpiLocateTableBySignature(
      ...
      &Index, (EFI_ACPI_DESCRIPTION_HEADER **)&McfgTable, &TableKey);
  if (EFI_ERROR(Status)) {
    DEBUG(
        (DEBUG_ERROR, "%a: Couldn't locate ACPI MCFG table! Status=%r\n",
         __func__, Status));
    return Status;
  }
  ...
  case ACPI_PCIE_ECAM_COMPAT_MODE_NXPMX6:
    ...
    Index  = 0;
    Status = AcpiLocateTableBySignature(
        mAcpiSdtProtocol, EFI_ACPI_6_3_FIXED_ACPI_DESCRIPTION_TABLE_SIGNATURE,
        &Index, &FadtTable, &TableKey);
    if (EFI_ERROR(Status)) {
      DEBUG(
          (DEBUG_ERROR, "%a: Couldn't locate ACPI FADT table! Status=%r\n",
           __func__, Status));
      return Status;
    }
  ...
  case ACPI_PCIE_ECAM_COMPAT_MODE_GRAVITON:
    ...
    CopyMem(
        McfgTable->Header.Header.OemId, "AMAZON",
        sizeof(McfgTable->Header.Header.OemId));
    McfgTable->Header.Header.OemTableId =
        SIGNATURE_64('G', 'R', 'A', 'V', 'I', 'T', 'O', 'N');
...
STATIC
VOID EFIAPI AcpiOsBootHandler(IN ACPI_OS_BOOT_TYPE OsType)
{
...
UINTN
EFIAPI
FindPeImageBase(EFI_PHYSICAL_ADDRESS Base)
{
...
BOOLEAN
EFIAPI
IsPeImageWinLoader(IN VOID *PeImage)
{
  CHAR8 *PdbStr;
  ...
  for (Index = 0; Index < PDB_NAME_MAX_LENGTH && PdbStr[Index] != '\0';
       Index++) {
...
EFI_STATUS
EFIAPI
AcpiExitBootServicesHook(IN EFI_HANDLE ImageHandle, IN UINTN MapKey)
{
  UINTN             ReturnAddress;
...
EFI_STATUS
EFIAPI
GetPciMem32TotalRange(OUT UINT64 *Base, OUT UINT64 *Size)
{
  EFI_STATUS           Status;
  UINTN                Index;
  EFI_HANDLE          *Handles;
  ...
  Status = gBS->LocateHandleBuffer(
      ByProtocol, &gEfiPciIoProtocolGuid, NULL, &HandleCount, &Handles);
  if (EFI_ERROR(Status)) {
    return Status;
  }
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    Status = gBS->HandleProtocol(
        Handles[Index], &gEfiPciIoProtocolGuid, (VOID **)&PciIo);
    ...
    Status = PciIo->Pci.Read(
        PciIo, EfiPciIoWidthUint32, 0, sizeof(PciConfigHeader) / sizeof(UINT32),
        &PciConfigHeader);
    if (EFI_ERROR(Status) ||
        (!IS_PCI_P2P(&PciConfigHeader) && !IS_PCI_P2P_SUB(&PciConfigHeader))) {
      continue;
    }

    MemoryBase = 0;
    PciIo->Pci.Read(
        PciIo, EfiPciIoWidthUint16, OFFSET_OF(PCI_TYPE01, Bridge.MemoryBase), 1,
        &MemoryBase);
    MemoryBase <<= 16;
    ...
    MemoryLimit = 0;
    PciIo->Pci.Read(
        PciIo, EfiPciIoWidthUint16, OFFSET_OF(PCI_TYPE01, Bridge.MemoryLimit),
        1, &MemoryLimit);
    MemoryLimit <<= 16;
...
//
STATIC
VOID EFIAPI AdjustPciReservedMemory(VOID)
{
  EFI_STATUS Status;
  ...
  if (mIsAcpiEnabled) {
    ...
    if (EFI_ERROR(Status)) {
      DEBUG(
          (DEBUG_ERROR, "%a: Failed to get Mem32 region. Status=%r\n", __func__,
           Status));
    ...
    DEBUG(
        (DEBUG_INFO, "%a: Mem32 Base: 0x%lx, Size: 0x%lx\n", __func__,
         PciMem32Base, PciMem32Size));

    if ((PciMem32Base < MemoryToReclaimBase) ||
        (PciMem32Size > MemoryToReclaimSize)) {
      ASSERT(FALSE);
      DEBUG(
          (DEBUG_ERROR, "%a: Mem32 region out of reserved bounds!", __func__));
    ...
    if (PciMem32PreferredSize <= MemoryToReclaimSize) {
      if (PciMem32Size < PciMem32PreferredSize) {
        DEBUG(
            (DEBUG_INFO, "%a: Mem32 Preferred Size: 0x%lx\n", __func__,
      ...
      DEBUG(
          (DEBUG_ERROR, "%a: Mem32 Preferred Size too large: 0x%lx\n", __func__,
           PciMem32PreferredSize));
    }
...
ReclaimMemoryExit:
  if ((mSystemMemorySize > PCI_RESERVED_MEM32_BASE) &&
      (MemoryToReclaimSize > 0)) {
    DEBUG(
        (DEBUG_INFO, "%a: Reclaiming system RAM - Base: 0x%lx, Size: 0x%lx\n",
         __func__, MemoryToReclaimBase, MemoryToReclaimSize));

    Status = gDS->AddMemorySpace(
        EfiGcdMemoryTypeSystemMemory, MemoryToReclaimBase, MemoryToReclaimSize,
    ...
    Status = gDS->SetMemorySpaceAttributes(
        MemoryToReclaimBase, MemoryToReclaimSize, EFI_MEMORY_WB);
    ASSERT_EFI_ERROR(Status);
  }
}
...
InstallAcpiTables(VOID)
{
  ...
  Status = gBS->LocateProtocol(
      &gEfiAcpiSdtProtocolGuid, NULL, (VOID **)&mAcpiSdtProtocol);
  if (EFI_ERROR(Status)) {
    DEBUG(
        (DEBUG_ERROR, "%a: Couldn't locate gEfiAcpiSdtProtocolGuid!\n",
         __func__));
    return Status;
  }
  ...
  Status     = AcpiLocateTableBySignature(
      ...
      &TableIndex, &mDsdtTable, &TableKey);
  if (EFI_ERROR(Status)) {
...
VOID EFIAPI ApplyConfigTableVariables(VOID)
{
  EFI_STATUS Status;
  EFI_EVENT  Event;
  ...
  Status = gBS->CreateEventEx(
      EVT_NOTIFY_SIGNAL, TPL_CALLBACK, OnReadyToBoot, NULL,
...
VOID EFIAPI SetupConfigTableVariables(VOID)
{
  EFI_STATUS Status;
  UINTN      Size;
  UINT32     Var32;
  ...
  AcpiPcie32BitBarSpaceSizeMB.Value =
      ACPI_PCIE_32_BIT_BAR_SPACE_SIZE_MB_DEFAULT;

  Size   = sizeof(ACPI_SD_COMPAT_MODE_VARSTORE_DATA);
  Status = gRT->GetVariable(
      L"AcpiSdCompatMode", &gRpiPlatformFormSetGuid, NULL, &Size,
      &AcpiSdCompatMode);
  if (EFI_ERROR(Status)) {
    Status = gRT->SetVariable(
        L"AcpiSdCompatMode", &gRpiPlatformFormSetGuid,
        EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS |
            EFI_VARIABLE_RUNTIME_ACCESS,
        Size, &AcpiSdCompatMode);
    ASSERT_EFI_ERROR(Status);
  }

  Size   = sizeof(ACPI_SD_LIMIT_UHS_VARSTORE_DATA);
  Status = gRT->GetVariable(
      L"AcpiSdLimitUhs", &gRpiPlatformFormSetGuid, NULL, &Size,
      &AcpiSdLimitUhs);
  if (EFI_ERROR(Status)) {
    Status = gRT->SetVariable(
        L"AcpiSdLimitUhs", &gRpiPlatformFormSetGuid,
        EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS |
            EFI_VARIABLE_RUNTIME_ACCESS,
        Size, &AcpiSdLimitUhs);
    ASSERT_EFI_ERROR(Status);
  ...
  Status = gRT->GetVariable(
      L"AcpiPcieEcamCompatMode", &gRpiPlatformFormSetGuid, NULL, &Size,
      &AcpiPcieEcamCompatMode);
  if (EFI_ERROR(Status)) {
    Status = gRT->SetVariable(
        L"AcpiPcieEcamCompatMode", &gRpiPlatformFormSetGuid,
        EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS |
            EFI_VARIABLE_RUNTIME_ACCESS,
        Size, &AcpiPcieEcamCompatMode);
    ASSERT_EFI_ERROR(Status);
  }

  Size   = sizeof(ACPI_PCIE_32_BIT_BAR_SPACE_SIZE_MB_VARSTORE_DATA);
  Status = gRT->GetVariable(
      L"AcpiPcie32BitBarSpaceSizeMB", &gRpiPlatformFormSetGuid, NULL, &Size,
      &AcpiPcie32BitBarSpaceSizeMB);
  if (EFI_ERROR(Status)) {
    Status = gRT->SetVariable(
        L"AcpiPcie32BitBarSpaceSizeMB", &gRpiPlatformFormSetGuid,
        EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS |
            EFI_VARIABLE_RUNTIME_ACCESS,
        Size, &AcpiPcie32BitBarSpaceSizeMB);
    ASSERT_EFI_ERROR(Status);
  }

  Size   = sizeof(UINT32);
  Status = gRT->GetVariable(
      L"SystemTableMode", &gRpiPlatformFormSetGuid, NULL, &Size, &Var32);
  if (EFI_ERROR(Status)) {
