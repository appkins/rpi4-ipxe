...
**/
BOOLEAN
ProbeRedfishCredentialBootstrap(VOID)
{
  ...
  Status = gBS->LocateProtocol(
      &gEdkIIRedfishCredential2ProtocolGuid, NULL,
  ...
  Status = CredentialProtocol->GetAuthInfo(
      CredentialProtocol, &AuthMethod, &UserName, &Password);
  if (!EFI_ERROR(Status)) {
...
caller has to free this memory using FreePool().

  @retval EFI_NOT_FOUND   No Redfish host interface descriptor provided on this
platform.
  ...
  while (TRUE) {
    if (ThisInstance->IsOnBoardNic && ThisInstance->IsSupportedHostInterface) {
      *DeviceType = REDFISH_HOST_INTERFACE_DEVICE_TYPE_PCI_PCIE_V2;
      ...
      // Fill up REDFISH_INTERFACE_DATA defined in Redfish host interface spec
      // v1.3 Allocate memory for the interface data structure
      ...
      InterfaceData->DeviceDescriptor.PciPcieDeviceV2.Length =
          PCI_INTERFACE_DEVICE_DESCRIPTOR_V2_SIZE;
      InterfaceData->DeviceDescriptor.PciPcieDeviceV2.VendorId =
          ThisInstance->PciVendorId;
      InterfaceData->DeviceDescriptor.PciPcieDeviceV2.DeviceId =
          ThisInstance->PciDeviceId;
      InterfaceData->DeviceDescriptor.PciPcieDeviceV2.SubsystemVendorId =
          ThisInstance->PciSubsystemVendorId;
      InterfaceData->DeviceDescriptor.PciPcieDeviceV2.SubsystemId =
          ThisInstance->PciSubsystemId;
      CopyMem(
      ...
      InterfaceData->DeviceDescriptor.PciPcieDeviceV2.SegmemtGroupNumber =
          ThisInstance->PciSegmentNumber;
      InterfaceData->DeviceDescriptor.PciPcieDeviceV2.BusNumber =
          ThisInstance->PciBusNumber;
      InterfaceData->DeviceDescriptor.PciPcieDeviceV2.DeviceFunctionNumber =
          ThisInstance->PciDeviceFunctionNumber;
    ...
    ThisInstance = (HOST_INTERFACE_RPI_PCI_NIC_INFO *)GetNextNode(
        &mRpiPciNic, &ThisInstance->NextInstance);
  }
...
/**
  ...
  Caller should pass NULL in ProtocolRecord to retrieve the first protocol
record. Then continuously pass previous ProtocolRecord for retrieving the next
ProtocolRecord.

  @param[in, out] ProtocolRecord  Pointer to retrieve the first or the next
protocol record. caller has to free the new protocol record returned from this
function using FreePool().
...
// Note: These PCDs need to be defined in the platform's .dec file
      ...
      ThisProtocolRecord =
          (MC_HOST_INTERFACE_PROTOCOL_RECORD *)AllocateZeroPool(
              sizeof(MC_HOST_INTERFACE_PROTOCOL_RECORD) - 1 +
              sizeof(REDFISH_OVER_IP_PROTOCOL_DATA) - 1 + HostNameLength);
      if (ThisProtocolRecord == NULL) {
        DEBUG(
            (DEBUG_ERROR, "    Allocate memory fail for "
      ...
      ThisProtocolRecord->ProtocolTypeDataLen =
          sizeof(REDFISH_OVER_IP_PROTOCOL_DATA) - 1 + HostNameLength;
      ...
      if ((PcdGetPtr(PcdRedfishServiceUuid) != NULL) &&
        ...
        StrToGuid(
            (CONST CHAR16 *)PcdGetPtr(PcdRedfishServiceUuid),
            &RedfishOverIpData->ServiceUuid);
        DEBUG(
            (DEBUG_REDFISH_HOST_INTERFACE, " Service UUID: %g",
             &RedfishOverIpData->ServiceUuid));
      }
#endif
      ...
      RedfishOverIpData->RedfishServiceIpDiscoveryType =
          ThisInstance->IpAssignedType;

      // HostIpAddressFormat and RedfishServiceIpAddressFormat, only support
      // IPv4 for now.
      ...
      // RedfishServiceIpPort - Read from EFI variable first
      EFI_STATUS VarStatus;
      UINTN      VarSize;
      UINT16     RedfishPortVar = 0;

      VarSize   = sizeof(RedfishPortVar);
      VarStatus = gRT->GetVariable(
          L"RedfishServiceIpPort", &gConfigDxeFormSetGuid, NULL, &VarSize,
          &RedfishPortVar);

      if (!EFI_ERROR(VarStatus) && (RedfishPortVar > 0)) {
        RedfishOverIpData->RedfishServiceIpPort = RedfishPortVar;
      }
      else {
#ifndef PcdRedfishServicePort
        RedfishOverIpData->RedfishServiceIpPort =
            5000; // Match the simulator port
#else
        RedfishOverIpData->RedfishServiceIpPort =
      ...
      CopyMem(
          ...
          (VOID *)HostNameString, HostNameLength);
...
RetrieveRpiPciNicInfo(VOID)
{
  ...
  while (TRUE) {
    if (ThisInstance->IsOnBoardNic) {
      // Since this is the only network interface available on Raspberry Pi,
      // we should support it for Redfish Host Interface
      ...
      //
      // Read Redfish service configuration from EFI variables first,
      // fall back to PCDs and then hardcoded defaults if variables don't exist
      //
      EFI_STATUS Status;
      UINTN      Size;
      CHAR8      RedfishIpAddressVar[16] = "";
      UINT16     RedfishPortVar          = 0;

      // Try to read IP address from EFI variable
      Size   = sizeof(RedfishIpAddressVar);
      Status = gRT->GetVariable(
          L"RedfishServiceIpAddress", &gConfigDxeFormSetGuid, NULL, &Size,
          RedfishIpAddressVar);

      if (!EFI_ERROR(Status) && (AsciiStrLen(RedfishIpAddressVar) > 0)) {
        // Parse IP address from variable (format: "192.168.1.10")
        CHAR8 *Token = AsciiStrStr(RedfishIpAddressVar, ".");
        if (Token != NULL) {
          ThisInstance->RedfishIpAddressIpv4[0] =
              (UINT8)AsciiStrDecimalToUintn(RedfishIpAddressVar);
          Token++;
          ThisInstance->RedfishIpAddressIpv4[1] =
              (UINT8)AsciiStrDecimalToUintn(Token);
          Token = AsciiStrStr(Token, ".") + 1;
          ThisInstance->RedfishIpAddressIpv4[2] =
              (UINT8)AsciiStrDecimalToUintn(Token);
          Token = AsciiStrStr(Token, ".") + 1;
          ThisInstance->RedfishIpAddressIpv4[3] =
              (UINT8)AsciiStrDecimalToUintn(Token);
        }
      }
      else {
        // Fall back to default IP address (simulator)
        ThisInstance->RedfishIpAddressIpv4[0] = 10;
        ThisInstance->RedfishIpAddressIpv4[1] = 0;
        ThisInstance->RedfishIpAddressIpv4[2] = 198;
        ThisInstance->RedfishIpAddressIpv4[3] = 24;
      }

      // Try to read port from EFI variable
      Size   = sizeof(RedfishPortVar);
      Status = gRT->GetVariable(
          L"RedfishServiceIpPort", &gConfigDxeFormSetGuid, NULL, &Size,
          &RedfishPortVar);

      if (!EFI_ERROR(Status) && (RedfishPortVar > 0)) {
        // Port will be used later in the protocol data function
        DEBUG(
            (DEBUG_REDFISH_HOST_INTERFACE,
             "    Using Redfish port from variable: %d\n", RedfishPortVar));
      }
      else {
        // Fall back to default port (simulator)
        RedfishPortVar = 5000;
        DEBUG(
            (DEBUG_REDFISH_HOST_INTERFACE,
             "    Using default Redfish port: %d\n", RedfishPortVar));
      }

#ifdef PcdRedfishUseStaticIp
      if (PcdGetBool(PcdRedfishUseStaticIp)) {
        ThisInstance->IpAssignedType =
            REDFISH_HOST_INTERFACE_HOST_IP_ASSIGNMENT_TYPE_STATIC;

        // Configure static IP addresses from PCDs or use defaults
        // These should be configurable via platform PCDs in a real
        // implementation
...
#endif
      {
        // Use DHCP for more flexible network configuration
        ThisInstance->IpAssignedType =
            REDFISH_HOST_INTERFACE_HOST_IP_ASSIGNMENT_TYPE_DHCP;

        // For DHCP, the host gets its IP automatically but still needs to know
        // Redfish service location
        ZeroMem(
            ThisInstance->HostIpAddressIpv4,
            sizeof(ThisInstance->HostIpAddressIpv4));
        ZeroMem(
            ThisInstance->SubnetMaskIpv4, sizeof(ThisInstance->SubnetMaskIpv4));
        ZeroMem(ThisInstance->GatewayIpv4, sizeof(ThisInstance->GatewayIpv4));
      }

      ThisInstance->VLanId = 0; // No VLAN by default

      DEBUG(
          (DEBUG_REDFISH_HOST_INTERFACE,
           "    Raspberry Pi network interface configured with %a IP "
           "assignment\n",
           (ThisInstance->IpAssignedType ==
            REDFISH_HOST_INTERFACE_HOST_IP_ASSIGNMENT_TYPE_DHCP)
               ? "DHCP"
               : "Static"));
      DEBUG((
          DEBUG_REDFISH_HOST_INTERFACE, "    Redfish service IP: %d.%d.%d.%d\n",
          ThisInstance->RedfishIpAddressIpv4[0],
          ThisInstance->RedfishIpAddressIpv4[1],
          ThisInstance->RedfishIpAddressIpv4[2],
          ThisInstance->RedfishIpAddressIpv4[3]));
...
PciNicGetNextMsgPciDp(IN EFI_DEVICE_PATH_PROTOCOL *ThisDevicePath)
{
  ...
  while (TRUE) {
    ...
    if ((ThisDevicePath->Type == HARDWARE_DEVICE_PATH) &&
        (ThisDevicePath->SubType == HW_PCI_DP)) {
...
EFI_STATUS
PciNicSearchPciIo(
    IN EFI_DEVICE_PATH_PROTOCOL *PciDevicePath, OUT EFI_PCI_IO_PROTOCOL **PciIo)
{
  EFI_STATUS                Status;
  UINTN                     BufferSize;
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Entry.\n", __func__));
  DEBUG(
      (DEBUG_REDFISH_HOST_INTERFACE, "Device path on the EFI handle which has "
                                     "PciIo and SNP installed on it.\n"));
  ...
  *PciIo       = NULL;
  Status       = gBS->LocateHandle(
      ByProtocol, &gEfiPciIoProtocolGuid, NULL, &BufferSize, NULL);
  if (Status == EFI_BUFFER_TOO_SMALL) {
    DEBUG(
        (DEBUG_REDFISH_HOST_INTERFACE, "  %d PciIo protocol instances.\n",
    ...
    Status = gBS->LocateHandle(
        ByProtocol, &gEfiPciIoProtocolGuid, NULL, &BufferSize, HandleBuffer);
    if (EFI_ERROR(Status)) {
  ...
  for (Index = 0; Index < (BufferSize / sizeof(EFI_HANDLE)); Index++) {
    ...
    DEBUG(
        (DEBUG_REDFISH_HOST_INTERFACE,
         "Device path on #%d instance of PciIo.\n", Index));
    ...
    if (DevicePathStr != NULL) {
      ...
      DEBUG(
          (DEBUG_ERROR,
           "Failed to convert device path on #%d instance of PciIo.\n", Index));
      continue;
    }

    // Compare device paths to find matching PCI IO protocol
    // For a more robust implementation, you would want to do proper device path
    // comparison For now, we'll try to match any PCI IO protocol as a
    // simplified approach
    if (DevicePathStr != NULL) {
      // Simple approach: if we found a PCI device path, try to get its PCI IO
      // protocol
      Status = gBS->HandleProtocol(
      ...
      if (!EFI_ERROR(Status)) {
        DEBUG(
            (DEBUG_REDFISH_HOST_INTERFACE,
             "EFI handle with PciIo is found at #%d instance.\n", Index));
        FreePool(HandleBuffer);
        return EFI_SUCCESS;
      }
    }
...
**/
EFI_STATUS
IdentifyNetworkMessageDevicePath(IN EFI_DEVICE_PATH_PROTOCOL *PciDevicePath)
{
  ...
  while (TRUE) {
    ...
    if (IsDevicePathEnd(DevicePath)) {
      DEBUG(
          (DEBUG_REDFISH_HOST_INTERFACE,
           "MAC address device path is not found on this handle.\n"));
      break;
    }

    if ((DevicePath->Type == MESSAGING_DEVICE_PATH) &&
        (DevicePath->SubType == MSG_MAC_ADDR_DP)) {
      DevicePath = NextDevicePathNode(
          DevicePath); // Advance to next device path protocol.
      if (IsDevicePathEnd(DevicePath)) {
        DEBUG(
            (DEBUG_REDFISH_HOST_INTERFACE,
             "IPv4 device path is not found on this handle.\n"));
        break;
      }
...
IdentifyRpiOnBoardPciNic(
    IN EFI_HANDLE Handle, IN EFI_DEVICE_PATH_PROTOCOL *PciDevicePath)
{
  UINTN                            Index;
  EFI_STATUS                       Status;
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Entry.\n", __func__));
  Status = gBS->HandleProtocol(
      Handle, &gEfiSimpleNetworkProtocolGuid, (VOID **)&Snp);
  if (EFI_ERROR(Status)) {
  ...
  if (RpiPciNic == NULL) {
    DEBUG(
        (DEBUG_ERROR, "    Failed to allocate memory for "
  ...
  CopyMem(
      (VOID *)RpiPciNic->MacAddress, (VOID *)&Snp->Mode->CurrentAddress,
      RpiPciNic->MacAddressSize);
  DEBUG(
      (DEBUG_REDFISH_HOST_INTERFACE,
       "    MAC address (in size %d) for this SNP instance:\n",
       RpiPciNic->MacAddressSize));
  for (Index = 0; Index < RpiPciNic->MacAddressSize; Index++) {
    DEBUG(
        (DEBUG_REDFISH_HOST_INTERFACE, "%02x ",
         *(RpiPciNic->MacAddress + Index)));
  }
  ...
  RpiPciNic->PciDeviceFunctionNumber =
      (UINT8)((DeviceNumber << 3) | FunctionNumber);
  ...
  if ((RpiPciNic->PciVendorId == 0x14E4) || // Broadcom
      (RpiPciNic->PciVendorId ==
       0x0424)) { // Microchip (for USB-Ethernet bridge)
    RpiPciNic->IsOnBoardNic = TRUE;
    DEBUG(
        (DEBUG_REDFISH_HOST_INTERFACE,
         "    Raspberry Pi on-board PCI NIC is found (Vendor: 0x%04x).\n",
         RpiPciNic->PciVendorId));
  }
  else {
    ...
    DEBUG(
        (DEBUG_REDFISH_HOST_INTERFACE,
         "    Unknown vendor 0x%04x - treating as potential on-board NIC for "
         "development.\n",
         RpiPciNic->PciVendorId));
    RpiPciNic->IsOnBoardNic = TRUE;
  }
...
**/
EFI_STATUS
CheckRpiPciNicOnHandles(IN UINTN HandleNumber, IN EFI_HANDLE *HandleBuffer)
{
  UINTN                     Index;
  EFI_STATUS                Status;
  EFI_DEVICE_PATH_PROTOCOL *DevicePath;
  ...
  DEBUG(
      (DEBUG_MANAGEABILITY, "%a: Entry, #%d SNP handle\n", __func__,
       HandleNumber));

  GotRpiPciNic = FALSE;
  for (Index = 0; Index < HandleNumber; Index++) {
    DEBUG(
        (DEBUG_MANAGEABILITY, "    Locate device path on handle 0x%08x\n",
         *(HandleBuffer + Index)));
    Status = gBS->HandleProtocol(
        *(HandleBuffer + Index), &gEfiDevicePathProtocolGuid,
        (VOID **)&DevicePath);
    if (EFI_ERROR(Status)) {
      DEBUG(
          (DEBUG_ERROR, "    Failed to locate device path on %d handle.\n",
           Index));
      continue;
    }
    ...
    while (TRUE) {
      if ((DevicePath->Type == HARDWARE_DEVICE_PATH) &&
        ...
        if (!EFI_ERROR(Status)) {
          Status =
              IdentifyRpiOnBoardPciNic(*(HandleBuffer + Index), DevicePath);
          if (!EFI_ERROR(Status)) {
            GotRpiPciNic = TRUE;
          }
        }
  ...
  DEBUG(
      (DEBUG_MANAGEABILITY,
       "No Raspberry Pi on-board PCI NIC found on SNP handles\n"));
  return EFI_NOT_FOUND;
}
...
**/
EFI_STATUS
CheckRpiPciNic(VOID *Registration)
{
  EFI_STATUS  Status;
  EFI_HANDLE  Handle;
  UINTN       BufferSize;
  ...
  DEBUG(
      (DEBUG_MANAGEABILITY, "%a: Entry, the registration key - 0x%08x.\n",
  ...
  do {
    ...
    Status     = gBS->LocateHandle(
        ...
        &gEfiSimpleNetworkProtocolGuid, Registration, &BufferSize, NULL);
    if (Status == EFI_BUFFER_TOO_SMALL) {
      DEBUG(
          (DEBUG_REDFISH_HOST_INTERFACE, "    %d SNP protocol instance(s).\n",
           BufferSize / sizeof(EFI_HANDLE)));
      HandleBuffer = AllocateZeroPool(BufferSize);
      if (HandleBuffer == NULL) {
        DEBUG(
            (DEBUG_ERROR, "    Failed to allocate buffer for the handles.\n"));
        return EFI_OUT_OF_RESOURCES;
      }

      Status = gBS->LocateHandle(
          Registration == NULL ? ByProtocol : ByRegisterNotify,
          &gEfiSimpleNetworkProtocolGuid, Registration, &BufferSize,
          HandleBuffer);
      if (EFI_ERROR(Status)) {
      ...
      if (Registration != NULL) {
        DEBUG(
            (DEBUG_REDFISH_HOST_INTERFACE,
             "    No more newly installed SNP protocol for this registration - "
             "%r.\n",
             Status));
        return EFI_SUCCESS;
      }
    ...
    Status =
        CheckRpiPciNicOnHandles(BufferSize / sizeof(EFI_HANDLE), HandleBuffer);
    if (!EFI_ERROR(Status)) {
      // Retrieve the rest of Raspberry Pi PCI NIC information for Redfish over
      // IP information and PCI Network Interface V2.
      Status = RetrieveRpiPciNicInfo();
      if (!EFI_ERROR(Status)) {
        DEBUG(
            (DEBUG_REDFISH_HOST_INTERFACE,
             "    Install protocol to notify the platform Redfish Host "
             "Interface information is ready.\n"));
        Status = gBS->InstallProtocolInterface(
            &Handle, &mPlatformHostInterfaceRpiPciNicReadinessGuid,
...
**/
VOID EFIAPI
PlatformHostInterfaceSnpCallback(IN EFI_EVENT Event, IN VOID *Context)
{
  DEBUG((DEBUG_MANAGEABILITY, "%a: Entry.\n", __func__));
    ...
    OUT EFI_GUID **InformationReadinessGuid)
{
  ...
  *InformationReadinessGuid = NULL;
  ...
  if (Status == EFI_NOT_FOUND) {
    DEBUG(
        (DEBUG_REDFISH_HOST_INTERFACE,
         "%a: Raspberry Pi PCI NIC is not found. Register the notification.\n",
         __func__));

    // Register the notification of SNP installation.
    Status = gBS->CreateEvent(
        EVT_NOTIFY_SIGNAL, TPL_CALLBACK, PlatformHostInterfaceSnpCallback, NULL,
        &mPlatformHostInterfaceSnpEvent);
    if (EFI_ERROR(Status)) {
      DEBUG(
          (DEBUG_ERROR,
           "%a: Fail to create event for the installation of SNP protocol.",
           __func__));
      return Status;
    }

    Status = gBS->RegisterProtocolNotify(
        &gEfiSimpleNetworkProtocolGuid, mPlatformHostInterfaceSnpEvent,
        &mPlatformHostInterfaceSnpRegistration);
    if (EFI_ERROR(Status)) {
      DEBUG(
          (DEBUG_ERROR,
           "%a: Fail to register event for the installation of SNP protocol.",
           __func__));
      return Status;
    }
  ...
  DEBUG(
      (DEBUG_ERROR,
       "%a: Something wrong when looking for Raspberry Pi PCI NIC.\n",
       __func__));
  return Status;
}
...
allocated memory for serial number.
  @retval EFI_SUCCESS         Serial number is returned.
  @retval Others              Failed to get the serial number
**/
EFI_STATUS
RedfishPlatformHostInterfaceSerialNumber(OUT CHAR8 **SerialNumber)
{
