...
#include "PlatformHostInterfaceBmcPciNicLib.h"

static EFI_GUID  mPlatformHostInterfaceBmcPciNicReadinessGuid =
  BMC_PCI_NIC_HOST_INTERFASCE_READINESS_GUID;
static EFI_EVENT  mPlatformHostInterfaceSnpEvent         = NULL;
static VOID       *mPlatformHostInterfaceSnpRegistration = NULL;

static LIST_ENTRY  mBmcPciNic;
...
{
  HOST_INTERFACE_BMC_PCI_NIC_INFO  *ThisInstance;
  REDFISH_INTERFACE_DATA           *InterfaceData;

  DEBUG ((DEBUG_MANAGEABILITY, "%a: Entry\n", __func__));

  if (IsListEmpty (&mBmcPciNic)) {
    return EFI_NOT_FOUND;
  }

  // Check if BMC exposed PCI NIC is found and ready for using.
  ThisInstance = (HOST_INTERFACE_BMC_PCI_NIC_INFO *)GetFirstNode (&mBmcPciNic);
  while (TRUE) {
    if (ThisInstance->IsExposedByBmc && ThisInstance->IsSuppportedHostInterface) {
      *DeviceType = REDFISH_HOST_INTERFACE_DEVICE_TYPE_PCI_V2;

      // Fill up REDFISH_INTERFACE_DATA defined in Redfish host interface spec v1.3
      InterfaceData = (REDFISH_INTERFACE_DATA *)AllocateZeroPool (PCI_INTERFACE_DEVICE_DESCRIPTOR_V2_SIZE_1_3);
      if (InterfaceData == NULL) {
        DEBUG ((DEBUG_ERROR, "Failed to allocate memory for REDFISH_INTERFACE_DATA\n"));
        return EFI_OUT_OF_RESOURCES;
      }

      InterfaceData->DeviceType                                   = REDFISH_HOST_INTERFACE_DEVICE_TYPE_PCI_V2;
      InterfaceData->DeviceDescriptor.PciDeviceV2.Length          = PCI_INTERFACE_DEVICE_DESCRIPTOR_V2_SIZE_1_3;
      InterfaceData->DeviceDescriptor.PciDeviceV2.IdVendor        = ThisInstance->PciVendorId;
      InterfaceData->DeviceDescriptor.PciDeviceV2.IdProduct       = ThisInstance->PciProductId;
      InterfaceData->DeviceDescriptor.PciDeviceV2.SerialNumberStr = 0;
      CopyMem (
        (VOID *)&InterfaceData->DeviceDescriptor.PciDeviceV2.MacAddress,
        (VOID *)ThisInstance->MacAddress,
        sizeof (InterfaceData->DeviceDescriptor.PciDeviceV2.MacAddress)
        );
      InterfaceData->DeviceDescriptor.PciDeviceV2.Characteristics              |= (UINT16)ThisInstance->CredentialBootstrapping;
      InterfaceData->DeviceDescriptor.PciDeviceV2.CredentialBootstrappingHandle = 0;
      *DeviceDescriptor                                                         = InterfaceData;
      DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "    REDFISH_INTERFACE_DATA is returned successfully.\n"));
      return EFI_SUCCESS;
    }

    if (IsNodeAtEnd (&mBmcPciNic, &ThisInstance->NextInstance)) {
      break;
    }

    ThisInstance = (HOST_INTERFACE_BMC_PCI_NIC_INFO *)
                   GetNextNode (&mBmcPciNic, &ThisInstance->NextInstance);
  }
  ...
  )
{
  ...
  if (IsListEmpty (&mBmcPciNic) || (IndexOfProtocolData > 0)) {
    return EFI_NOT_FOUND;
  }
...
/**
  This function retrieve the information of BMC PCI NIC.

  @retval EFI_SUCCESS      All necessary information is retrieved.
  @retval EFI_NOT_FOUND    There is no BMC exposed PCI NIC.
  @retval Others           Other errors.

**/
EFI_STATUS
RetrievedBmcPciNicInfo (
  VOID
  )
{
  EFI_STATUS                                      Status;
  ...
  EFI_PCI_DEVICE_DESCRIPTOR                       PciDeviceDescriptor;
  ...
  while (TRUE) {
    if (ThisInstance->IsExposedByBmc) {
      ...
      // The IP address at BMC PCI NIC host end is the IP address at BMC end minus 1.
      //
      ...
      //
      if (ThisInstance->ThisPciIo != NULL) {
        Status = ThisInstance->ThisPciIo->PciGetDeviceDescriptor (ThisInstance->ThisPciIo, &PciDeviceDescriptor);
        if (!EFI_ERROR (Status)) {
          DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "    PCI NIC Vendor ID: 0x%04x, Device ID: 0x%04x\n", PciDeviceDescriptor.IdVendor, PciDeviceDescriptor.IdProduct));
          ThisInstance->PciVendorId  = PciDeviceDescriptor.IdVendor;
          ThisInstance->PciProductId = PciDeviceDescriptor.IdProduct;
        } else {
          DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "    Fail to get PCI device descriptor.\n"));
        }
      }
...
/**
  ...
  don't have to sedn IPMI commands again if the PCI NIC is
  connected later.
...
/**
  ...
  mactched MAC addrss of BMC PCI NIC endpoint.

  @param[in] PciNicInfo  The instance of HOST_INTERFACE_BMC_PCI_NIC_INFO.

  @retval EFI_SUCCESS          Yes, PCI NIC exposed by BMC is found.
  @retval EFI_NOT_FOUND        No, PCI NIC exposed by BMC is not found
...
HostInterfaceIpmiCheckMacAddress (
  IN HOST_INTERFACE_BMC_PCI_NIC_INFO  *PciNicInfo
  )
{
  EFI_STATUS                                      Status;
       ...
       ChannelNum++)
  {
    ...
    if (IpmiLanMacAddressSize != 0) {
      ...
      //
      if ((IpmiLanMacAddressSize != PciNicInfo->MacAddressSize) ||
          (CompareMem (
             (VOID *)PciNicInfo->MacAddress,
             (VOID *)&IpmiLanChannelMacAddress.Addr,
             IpmiLanMacAddressSize - 1
             ) != 0) ||
          ((IpmiLanChannelMacAddress.Addr[IpmiLanMacAddressSize - 1] - 1) !=
           *(PciNicInfo->MacAddress + IpmiLanMacAddressSize - 1))
          )
      {
      ...
      PciNicInfo->IpmiLanChannelNumber = (UINT8)ChannelNum;
      PciNicInfo->IsExposedByBmc       = TRUE;
  ...
  )
{
  ...
  while (TRUE) {
    ...
    if ((ThisDevicePath->Type == MESSAGING_DEVICE_PATH) && (ThisDevicePath->SubType == MSG_PCI_DP)) {
...
/**
  ...
  @retval EFI_SUCCESS          Yes, PciIo protocl is found.
  @retval EFI_NOT_FOUND        No, PciIo protocl is not found
  @retval Others               Other errors.
  ...
  )
{
  ...
  EFI_DEVICE_PATH_PROTOCOL  *ThisPciDevicePath;
  EFI_DEVICE_PATH_PROTOCOL  *ThisPciDevicePathEnd;

  DEBUG ((DEBUG_MANAGEABILITY, "%a: Entry.\n", __func__));
  DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "Device path on the EFI handle which has PciIo and SNP instaleld on it.\n"));
  ...
  *PciIo       = NULL;
  ...
  if (Status == EFI_BUFFER_TOO_SMALL) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "  Falied to locate PciIo protocol handles.\n"));
  ...
  for (Index = 0; Index < (BufferSize/sizeof (EFI_HANDLE)); Index++) {
    ...
    // Search for the starting MSG_PCI_DP node.
    ThisPciDevicePath = PciDevicePath;
    if ((DevicePathType (ThisPciDevicePath) != MESSAGING_DEVICE_PATH) ||
        (DevicePathSubType (ThisPciDevicePath) != MSG_PCI_DP))
    {
      ThisPciDevicePath = PciNicGetNextMsgPciDp (ThisPciDevicePath);
      if (ThisPciDevicePath == NULL) {
        continue;
      }
    }

    if ((DevicePathType (ThisDevicePath) != MESSAGING_DEVICE_PATH) ||
        (DevicePathSubType (ThisDevicePath) != MSG_PCI_DP))
    {
      ThisDevicePath = PciNicGetNextMsgPciDp (ThisDevicePath);
      if (ThisDevicePath == NULL) {
        continue;
      }
    }

    // Search for the ending MSG_PCI_DP node.
    ThisDevicePathEnd    = ThisDevicePath;
    ThisPciDevicePathEnd = ThisPciDevicePath;
    while (TRUE) {
      TempDevicePath = PciNicGetNextMsgPciDp (ThisDevicePathEnd);
    ...
    while (TRUE) {
      TempDevicePath = PciNicGetNextMsgPciDp (ThisPciDevicePathEnd);
      if (TempDevicePath == NULL) {
        break;
      }

      ThisPciDevicePathEnd = TempDevicePath;
    }
    ...
    if (Length != ((UINTN)(UINT8 *)ThisPciDevicePathEnd + DevicePathNodeLength (ThisPciDevicePathEnd) - (UINTN)(UINT8 *)ThisPciDevicePath)) {
      continue;
    }

    if (CompareMem (
          (VOID *)ThisDevicePath,
          (VOID *)ThisPciDevicePath,
          Length
          ) == 0)
    {
      Status = EFI_SUCCESS;
      DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "EFI handle with the correct PciIo is found at #%d instance of PciIo.\n", Index));
      break;
    }
  }

  if (Status == EFI_SUCCESS) {
    // Locate PciIo from this handle.
    Status = gBS->HandleProtocol (
                    *(HandleBuffer + Index),
                    &gEfiPciIoProtocolGuid,
                    (VOID **)PciIo
...
/**
  This function identifies if the PCI NIC is exposed by BMC as
  ...
  )
{
  ...
  HOST_INTERFACE_BMC_PCI_NIC_INFO  *BmcPciNic;
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "    Failed to find PCIIO.\n"));
    return Status;
  }

  // Get the MAC address of this SNP instance.
  BmcPciNic = AllocateZeroPool (sizeof (HOST_INTERFACE_BMC_PCI_NIC_INFO));
  if (BmcPciNic == NULL) {
    DEBUG ((DEBUG_ERROR, "    Failed to allocate memory for HOST_INTERFACE_BMC_PCI_NIC_INFO.\n"));
    return EFI_OUT_OF_RESOURCES;
  }

  InitializeListHead (&BmcPciNic->NextInstance);
  BmcPciNic->MacAddressSize = Snp->Mode->HwAddressSize;
  BmcPciNic->MacAddress     = AllocatePool (BmcPciNic->MacAddressSize);
  if (BmcPciNic->MacAddress == NULL) {
  ...
  CopyMem (
    (VOID *)BmcPciNic->MacAddress,
    (VOID *)&Snp->Mode->CurrentAddress,
    BmcPciNic->MacAddressSize
    );
  DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "    MAC address (in size %d) for this SNP instance:\n", BmcPciNic->MacAddressSize));
  for (Index = 0; Index < BmcPciNic->MacAddressSize; Index++) {
    DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "%02x ", *(BmcPciNic->MacAddress + Index)));
  }

  DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "\n"));
  BmcPciNic->ThisSnp   = Snp;
  BmcPciNic->ThisPciIo = PciIo;

  Status = HostInterfaceIpmiCheckMacAddress (BmcPciNic);
  if (Status == EFI_SUCCESS) {
    BmcPciNic->IsExposedByBmc = TRUE;
    DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "    BMC exposed PCI NIC is found.\n"));
  } else {
    DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "    BMC exposed PCI NIC is not found.\n"));
  }

  InsertTailList (&mBmcPciNic, &BmcPciNic->NextInstance);
  return Status;
}

/**
  This function checks if the PCI NIC exposed by BMC
  ...
  )
{
  ...
  for (Index = 0; Index < HandleNumer; Index++) {
    ...
    // Check if this is an BMC exposed PCI NIC device.
    while (TRUE) {
      if ((DevicePath->Type == MESSAGING_DEVICE_PATH) && (DevicePath->SubType == MSG_PCI_DP)) {
        Status = IdentifyNetworkMessageDevicePath (DevicePath);
        if (!EFI_ERROR (Status)) {
          Status = IdentifyPciNicBmcChannel (*(HandleBuffer + Index), DevicePath);
          if (!EFI_ERROR (Status)) {
            GotBmcPciNic = TRUE;
          }
        }
  ...
  DEBUG ((DEBUG_MANAGEABILITY, "No BMC PCI NIC found on SNP handles\n"));
  return EFI_NOT_FOUND;
}
  ...
  )
{
  ...
  do {
    ...
    Status = CheckBmcPciNicOnHandles (BufferSize/sizeof (EFI_HANDLE), HandleBuffer);
    if (!EFI_ERROR (Status)) {
      // Retrieve the rest of BMC PCI NIC information for Redfish over IP information
      // and PCI Network Interface V2.
      Status = RetrievedBmcPciNicInfo ();
      if (!EFI_ERROR (Status)) {
        ...
        Status = gBS->InstallProtocolInterface (
                        &Handle,
                        &mPlatformHostInterfaceBmcPciNicReadinessGuid,
  ...
  )
{
  ...
  CheckBmcPciNic (mPlatformHostInterfaceSnpRegistration);
  return;
}
  ...
  )
{
  ...
  *InformationReadinessGuid = NULL;
  InitializeListHead (&mBmcPciNic);
  InitializeListHead (&mBmcIpmiLan);

  //
  // Check if PCI NIC exposed by BMC is already
  // connected.
  //
  Status = CheckBmcPciNic (NULL);
  if (!EFI_ERROR (Status)) {
    return EFI_ALREADY_STARTED;
  }

  if (Status == EFI_NOT_FOUND) {
    DEBUG ((DEBUG_REDFISH_HOST_INTERFACE, "%a: BMC PCI NIC is not found. Register the notification.\n", __func__));
    ...
    *InformationReadinessGuid = &mPlatformHostInterfaceBmcPciNicReadinessGuid;
    return EFI_SUCCESS;
  }

  DEBUG ((DEBUG_ERROR, "%a: Something wrong when look for BMC PCI NIC.\n", __func__));
  return Status;
}

/**
  Get PCI device serial number.
