/** @file
  ...
  with conditional authentication based on PcdRedfishServiceAuthenticationEnabled.
...
#define REDFISH_AUTH_ENABLED_VARIABLE_NAME L"RedfishServiceAuthenticationEnabled"
  ...
  )
{
  ...
  Status = gRT->GetVariable (
                  REDFISH_AUTH_ENABLED_VARIABLE_NAME,
                  &RpiVariablesGuid,
                  NULL,
                  &DataSize,
                  AuthEnabled
  ...
  if (EFI_ERROR (Status)) {
    ...
    *AuthEnabled = FALSE;
    DEBUG ((DEBUG_INFO, "%a: Using default authentication: Disabled\n", __func__));
    return EFI_SUCCESS;
  }

  DEBUG ((DEBUG_INFO, "%a: Authentication from variable: %a\n",
         __func__, *AuthEnabled ? "Enabled" : "Disabled"));
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (Status == EFI_BUFFER_TOO_SMALL) {
    ...
    if (!EFI_ERROR (Status) && (AsciiStrLen (VariableData) > 0)) {
      ...
      DEBUG ((DEBUG_INFO, "%a: Variable %s invalid, using PCD\n", __func__, VariableName));
    }
  } else {
    // Variable doesn't exist, use PCD
    SourceString = PcdString;
    DEBUG ((DEBUG_INFO, "%a: Variable %s not found, using PCD\n", __func__, VariableName));
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to get authentication setting - %r\n", __func__, Status));
    return Status;
  }
  ...
  if (!AuthEnabled) {
    ...
    *UserId = NULL;
    *Password = NULL;
    DEBUG ((DEBUG_INFO, "%a: Authentication disabled, using AuthMethodNone\n", __func__));
    return EFI_SUCCESS;
  }
  ...
  *AuthMethod = AuthMethodHttpBasic;
  ...
  Status = GetCredentialString (
             ...
             "admin",  // Default fallback user ID
             UserId
             );
  if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "EFI Redfish service is stopped (type: %d)\n", ServiceStopType));
