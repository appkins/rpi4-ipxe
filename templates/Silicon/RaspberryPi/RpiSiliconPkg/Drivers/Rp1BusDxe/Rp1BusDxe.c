...
VOID EFIAPI Rp1BusRegisterDwc3Controllers(IN RP1_BUS_DATA *Rp1Data)
{
  ...
  for (Index = 0; Index < ARRAY_SIZE(Dwc3Addresses); Index++) {
    ...
    if (EFI_ERROR(Status)) {
      DEBUG(
          (DEBUG_ERROR, "RP1: Failed to open DWC3 by controller. Status=%r\n",
           Status));
      continue;
    }
  }
...
{
  MmioWrite32(
      Rp1Data->PeripheralBase + RP1_PCIE_REG_SET +
    ...
    IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath)
{
  ...
  Status = gBS->OpenProtocol(
      ControllerHandle, &gEfiPciIoProtocolGuid, (VOID **)&PciIo,
...
Exit:
  gBS->CloseProtocol(
      ControllerHandle, &gEfiPciIoProtocolGuid, This->DriverBindingHandle,
      ControllerHandle);
    ...
    IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath)
{
  ...
  if (Rp1Data == NULL) {
    ...
    DEBUG(
        (DEBUG_ERROR, "RP1: Failed to allocate device context. Status=%r\n",
         Status));
    goto Fail;
  }
  ...
  Status = PciIo->GetBarAttributes(
      PciIo, RP1_PERIPHERAL_BAR_INDEX, NULL, (VOID **)&PeripheralDesc);
  if (EFI_ERROR(Status)) {
    DEBUG((
        DEBUG_ERROR, "RP1: Failed to get BAR attributes. Status=%r\n", Status));
    goto Fail;
  }
  ...
  if (EFI_ERROR(Status)) {
    DEBUG(
        (DEBUG_ERROR, "RP1: Failed to install bus protocol. Status=%r\n",
         Status));
    goto Fail;
  }

  DEBUG(
      (DEBUG_INFO, "RP1: chip id %x, peripheral base at CPU address 0x%lx\n",
    ...
    IN EFI_HANDLE DeviceHandle)
{
  ...
  Status = gBS->OpenProtocol(
      DeviceHandle, &gEfiDevicePathProtocolGuid,
      (VOID **)&NonDiscoverableDevicePath, This->DriverBindingHandle,
      ControllerHandle, EFI_OPEN_PROTOCOL_GET_PROTOCOL);
  if (EFI_ERROR(Status)) {
    ASSERT_EFI_ERROR(Status);
    return Status;
  ...
  if (EFI_ERROR(Status)) {
    gBS->OpenProtocol(
        ControllerHandle, &gRp1BusProtocolGuid, (VOID **)&Rp1Bus,
...
Rp1BusDriverBindingStop(
    IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
    IN UINTN NumberOfChildren, IN EFI_HANDLE *DeviceHandleBuffer)
{
  EFI_STATUS        Status;
  UINTN             Index;
  RP1_BUS_PROTOCOL *Rp1Bus;
  ...
  for (Index = 0; Index < NumberOfChildren; Index++) {
    ...
    Status = Rp1BusUnregisterNonDiscoverableDevice(
        This, ControllerHandle, DeviceHandleBuffer[Index]);
    if (EFI_ERROR(Status)) {
...
EFI_STATUS
EFIAPI
Rp1BusDxeEntryPoint(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
